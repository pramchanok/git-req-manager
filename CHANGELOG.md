...
- **ปรับปรุงรายงานทีมและรายงานนักพัฒนา**: รองรับการเลือกช่วง Daily, Weekly และ Monthly พร้อมกรองข้อมูลตามช่วงเวลาจริง และปรับปรุงประสิทธิภาพการรวมสถิติสมาชิก
- **ปรับปรุง Diff Viewer**: ตรวจจับภาษาจากนามสกุลไฟล์และทำ syntax highlighting ให้เหมาะสม พร้อมลดการประมวลผลซ้ำของ diff ขนาดใหญ่
- **ปรับปรุงการทำงานของ GitLab API**: เพิ่ม timeout, pagination ที่เป็นมาตรฐาน และจำกัด concurrency เพื่อให้การ sync มีเสถียรภาพขึ้น
- **ปรับปรุง Webhook และ IPC**: รวม lifecycle ของ polling/webhook/relay และจัดการ listener ของ renderer แบบเฉพาะตัวเพื่อไม่ให้ component อื่นได้รับผลกระทบ

### การแก้ไขข้อบกพร่อง (Bug Fixes)

- **แก้รายงานย้อนหลังนับ MR ไม่ครบ**: ไม่ใช้ขอบเขต `updated_before` ของ GitLab API ที่อาจตัด MR ซึ่งถูกอัปเดตหลังช่วงรายงาน แต่มี activity อยู่ในช่วงนั้น
- **ป้องกัน sync ซ้อนกัน**: scheduler จะไม่เริ่มรอบใหม่ขณะที่รอบเดิมยังทำงานอยู่ เพื่อป้องกัน race condition และ notification ซ้ำ
- **แก้การนำทางรายเดือนช่วงปลายเดือน**: วันที่ 29–31 จะไม่ทำให้การเปลี่ยนเดือนไปข้ามเดือนถัดไป
- **รักษาสถานะไฟล์ที่เคยดูแล้ว**: ข้อมูลรูปแบบเดิมใน localStorage จะไม่ถูกลบทิ้งระหว่าง migration
- **เพิ่มความปลอดภัยของ webhook และ token**: จำกัดขนาด payload, ตรวจ secret แบบ constant-time, จำกัด endpoint ที่ไม่มี secret ไว้ที่ loopback และตรวจสถานะการถอดรหัส token ก่อนใช้งาน
