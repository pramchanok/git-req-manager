...
- **คืนค่าไอคอน Tray App ดั้งเดิม**: คืนค่าภาพไอคอน Tray App กรอบสี่เหลี่ยมสีส้ม/เทา/ฟ้า (Squircle) ดั้งเดิมของแอป พร้อมยกเลิกการปรับ Template Image บน macOS เพื่อให้แสดงผลภาพไอคอนดั้งเดิมได้สมบูรณ์ทั้งบน Windows และ macOS
...
- **Redesign หน้า Lead Overview**: ปรับปรุงหน้าจอสรุปภาพรวมสำหรับ Team Lead ให้มีโครงสร้างการอ่านเป็นลำดับขั้น (Visual Hierarchy) ระดับพรีเมียม พร้อมการ์ด KPI สรุปผล 5 หมวดหลัก
- **ระบบเปรียบเทียบสมาชิกทีม (Executive Member Comparison Modal)**: เพิ่มหน้าต่าง Modal เปรียบเทียบผลงานของสมาชิกในทีม 2-3 คนแบบข้างต่อข้าง (Side-by-Side) พร้อมกราฟแท่งเปรียบเทียบกิจกรรม อัตรา Merge Success Rate (%) และเวลาเฉลี่ย (Avg Lead Time)
- **เพิ่มช่วงเวลารายปี (Yearly Timeframe)**: รองรับการเลือกดูรายงานและสถิติกิจกรรมแบบรายปี (Yearly) ทั้งในหน้า Dashboard หลัก รายงานสมาชิก และหน้าต่าง Compare
- **การเลือกสมาชิกในเปรียบเทียบแบบยืดหยุ่น**: ปรับให้เริ่มต้นแบบไม่ฟิกซ์ตัวเลือก และสามารถเลือก/สลับรายชื่อสมาชิกจากหน้าต่างกลางจอได้อย่างอิสระ
- **เพิ่มกราฟและตัวชี้วัดประสิทธิภาพ**: แสดงสัดส่วนผลงานในทีม (Team Workload Share %), สรุปโปรเจกต์ที่มีความเคลื่อนไหวสูงสุด (Top Active Projects) และรางวัลเกียรติยศสมาชิกประจำช่วงเวลา (Team Period Highlights)
...
- **ป้องกันปัญหาหน้าจอขาว (Blank Screen Crash Prevention)**: ครอบด้วย React `ModalErrorBoundary` และเพิ่ม Defensive Null Guarding สำหรับชื่อและรูปโปรไฟล์สมาชิก ป้องกันแอพล่มเมื่อพบข้อมูลผู้ใช้ไม่สมบูรณ์
- **แก้ไข Tooltip และ Native Popup**: ปรับปรุง Tooltip กราฟกิจกรรมให้จัดตำแหน่งตามดัชนีวัน (index-aware alignment) ไม่หลุดขอบ พร้อมลบ title attribute ซ้ำซ้อนของเบราว์เซอร์
...
- **แสดงผู้รับผิดชอบ Review และผู้ Merge อย่างชัดเจน**: หน้า MR แสดง avatar ของผู้ที่ถูก request ให้ review และคงข้อมูลนั้นไว้หลัง MR ถูก merge พร้อมแสดงผู้ที่ merge จริงแยกต่างหาก
- **เพิ่มรายงานประวัติส่วนตัว**: ผู้ที่ไม่มีสิทธิ์ดู Team Report สามารถเปิดดูประวัติ contribution ของตนเองได้ โดยไม่เห็นรายชื่อหรือ activity ของสมาชิกคนอื่น
...
- **เพิ่มความเสถียรของ GitLab Sync**: retry เฉพาะ network/timeout และ server error ชั่วคราวสำหรับ GET พร้อม backoff และแสดงข้อความข้อผิดพลาดที่ระบุสาเหตุได้ชัดเจนขึ้น
- **รักษาข้อมูลรอบล่าสุด**: เมื่อ sync รอบใหม่ล้มเหลว Dashboard ยังแสดงข้อมูลที่โหลดสำเร็จครั้งล่าสุด พร้อมแจ้งเตือนให้ลองใหม่
- **เพิ่มความสามารถหน้า Changes**: คัดลอก path ของไฟล์และเปิดไฟล์ใน IDE ที่ติดตั้งอยู่ได้ โดยจำกัด path ให้อยู่ภายใน local repository

### การแก้ไขข้อบกพร่อง (Bug Fixes)

- **ป้องกัน path traversal และ symlink escape**: ตรวจ canonical path และชนิดไฟล์ก่อนเปิดไฟล์จาก diff เพื่อไม่ให้เปิดไฟล์นอก repository ที่เลือกไว้
- **หยุดสร้าง shortcut อัตโนมัติ**: ไม่สร้าง Desktop หรือ Start Menu shortcut ซ้ำทุกครั้งที่เปิด ติดตั้ง หรืออัปเดตโปรแกรม
